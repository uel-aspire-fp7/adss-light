...
4. About the class "ProtectionConfiguration", please refer the class file. Generally speaking, it includes a series of protections with parameters.
  ...
  * 
  * 2016-03-21, this interface is created to be a backup for SPA to use the data classes in ADSS. 
  *   * */

public interface SPA2 {
	...
	//this function is to set a specific attack model which will be calculated for protection assessment
		...
		//metrics false=> the model is just a PN, the metrics/steps associations are determined by SPA2 
		public boolean initModel(ArrayList<eu.aspire_fp7.adss.akb.AttackPath> aps, boolean metrics);
		...
		//this method is to tell spa that the metrics data will come from the exiting files not the runtime ACTC. 15/04/2016, by gaofeng
		public boolean setMetricFile(String filefullpath);
		
		//all measure considered in this assessment, came from the Knowledge base 		
		public boolean initMeasureList(ArrayList<String> measurelist); 
		
		//all metrics(name + value) considered in this assessment, came from the Knowledge base 
		...
		public boolean initProtectionSolution(Solution ps);
		...
		//all metrics(name + value) considered in this assessment, came from the Knowledge base 
		boolean initMetrics(EList<Metric> metrics);

		boolean initMetrics(ArrayList<Metric> metricsafter, ArrayList<Metric> metricsbefore);

		boolean initMetrics(EList<Metric> metricsafter, EList<Metric> metricsbefore);

		boolean initModel(EList<AttackPath> apsin, boolean metrics);
